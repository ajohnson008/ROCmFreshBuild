...
# read pins from flake
pins=$(nix eval --raw "(import $HERE/flake.nix {}).pins")

# Fallback parsing: use grep/sed since pins structure is static
for name in rocm-systems rocm-cmake "ROCR-Runtime" clr llvm-project; do
  url=$(nix eval --raw "(import $HERE/flake.nix {}).pins.${name}.url")
