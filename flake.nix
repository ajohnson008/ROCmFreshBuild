{
  ...
  # ============================================================================
  # NOTE: By default ROCm sources are fetched directly from the tag (fetchFromGitHub)
  # to avoid locking large archives into the flake. For stricter reproducibility we
  # pin a small set of core ROCm repos (commit SHAs for `rocm-7.2.0`) as flake inputs
  # so that `flake.lock` captures their exact revisions. Large monorepos like
  # `rocm-libraries` are intentionally omitted to avoid flake lock timeouts.
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-24.11";
    nixpkgs-unstable.url = "github:NixOS/nixpkgs/nixos-unstable";
    flake-utils.url = "github:numtide/flake-utils";

    # NOTE: We centralize ROCm commit pins in a local tiny flake to avoid
    # having top-level flake inputs that point to non-flake repositories.
    # Use `pinned-rocm` (./pinned-rocm) to see/modify the exact commit SHAs.
    pinned-rocm.url = "path:./pinned-rocm";

    # NOTE: Tarball fetches and their sha256 values are recorded in
    # `pinned-rocm/sources.nix` (generated by `pinned-rocm/fill-shas.sh`).
    # Use `import ./pinned-rocm`.sources to access deterministic tarball
    # URLs and sha256 values from other derivations.
  };
